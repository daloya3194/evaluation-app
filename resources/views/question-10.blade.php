...
@section('content')
    <div class="container mx-auto mt-5">
        ...
        <h1 class="text-center text-5xl font-bold">Task 10</h1>
                        ...
                        <div class="grid grid-cols-6 gap-1">
                            @isset($cluster_1)
                                @foreach($cluster_1 as $image)
                                    <div class="bg-black">
                                        <input class="sr-only peer" onclick="verifyImage({{ $image->id }}, '{{ route('question-submit') }}', 11)" type="radio" value="{{ $image->id }}" name="image_id" id="{{ 'image_' . $image->id }}" required>
