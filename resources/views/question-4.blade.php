...
@section('content')
    <div class="container max-w-6xl mx-auto mt-20">
        ...
        <h1 class="text-center text-5xl font-bold">Task 4</h1>
        ...
        <div>
            <form action="{{ route('question-4-submit') }}" method="POST">
                @csrf
                <br>
                ...
                <div class="grid grid-cols-5 gap-3">
                    @isset($images)
                        @foreach($images as $image)
                            <div class="bg-black">
                                <input class="sr-only peer" onclick="verifyImage({{ $image->id }}, '{{ route('question-submit') }}', 5)" type="radio" value="{{ $image->id }}" name="image_id" id="{{ 'image_' . $image->id }}" required>
