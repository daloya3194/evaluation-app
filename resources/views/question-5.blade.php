...
@section('content')
    <div class="container mx-auto mt-20">
        ...
        <h1 class="text-center text-5xl font-bold">Task 5</h1>
                ...
                <div class="grid grid-cols-6 gap-3">
                    @isset($images)
                        @foreach($images as $image)
                            <div class="bg-black">
                                <input class="sr-only peer" onclick="verifyImage({{ $image->id }}, '{{ route('question-submit') }}', 6)" type="radio" value="{{ $image->id }}" name="image_id" id="{{ 'image_' . $image->id }}" required>
